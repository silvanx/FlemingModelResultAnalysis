...
def load_voltage_signal(filename: Path):
    data = NeoMatlabIO(filename).read()
    sig = data[0].segments[0].analogsignals[0]
...
def load_cortical_soma_voltage(dir: Path) -> tuple[np.ndarray, AnalogSignal]:
    ...
    sig = load_voltage_signal(filename)
    time = sig.times
...
def load_stn_soma_voltage(dir: Path) -> tuple[np.ndarray, AnalogSignal]:
    '''Reads STN soma voltage from a .mat file'''
    filename = dir / "STN_Pop" / "STN_Soma_v.mat"
...
def load_and_plot(dirname: Path, start=0, stop=None) -> None:
    ...
    axs[0].plot(controller_t[s:e], controller_b[s:e], color="darkmagenta", linewidth=5)
    ...
    axs[0].set_ylim([0, 4e-4])

    print(f"{dirname.name}: beta avg {np.average(controller_b)}; median {np.median(controller_b)}")

    axs[1].plot(time_lfp[s_lfp:e_lfp] / 1000, lfp[s_lfp:e_lfp])
    format_single_axis(axs[1], "Time [s]", "STN LFP [mV]", "STN LFP")

    axs[2].plot(time_dbs[s_dbs:e_dbs] / 1000, dbs[s_dbs:e_dbs])
    format_single_axis(axs[2], "Time [s]", "DBS amplitude [mA]", "DBS signal")
